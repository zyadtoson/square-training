...
css_to_add = """
      ...
      .team-cta { background: #25D366; color: #fff; text-decoration: none; padding: 10px; border-radius: 8px; display: block; text-align: center; font-weight: bold; transition: 0.2s; cursor: pointer; border: none; font-size: 1rem; }
...
t_sidebar_target = """            <div class="nav-item" onclick="switchTab('t-analytics', this)">
              ...
              <span class="n-text" style="flex: 1">تحليل الطلاب</span>
              <span class="badge" id="t-analytics-badge" style="display:none;">0</span>
            </div>"""
t_sidebar_add = """
            <div class="nav-item" onclick="switchTab('t-project', this)">
              <span class="n-icon">🚀</span>
              <span class="n-text" style="flex: 1">المشروع النهائي</span>
            </div>"""
...
s_sidebar_target = """            <div class="nav-item" onclick="switchTab('s-profile', this)">
              <span class="n-icon">��</span>
              <span class="n-text">الملف الشخصي</span>
            </div>"""
s_sidebar_add = """
...
t_tab_content = """        <!-- /ANALYTICS -->
        ...
        <div id="t-project" class="tab-content" style="padding-bottom: 40px;">
          <div class="dash">
            <div class="sec-head">
...
s_tab_content_target = """        <!-- /PROFILE -->"""
s_tab_content = """        <!-- /PROFILE -->

        <div id="s-project" class="tab-content" style="padding-bottom: 40px;">
          <div class="stu-dash">
            <div class="sec-head">
              <h2 class="sec-title">🎯 تفاصيل <em>المشروع النهائي</em></h2>
            </div>
...
modal_add = """            <div class="fg">
              ...
              <select id="ms-team" style="width: 100%; background: #1a1a1a; border: 1px solid #3a3a3a; border-radius: 8px; padding: 10px 14px; color: #eee; font-size: 0.9rem; direction: rtl; transition: border-color 0.2s;">
...
js_logic = """
      ...
      function getStudentTeam(student) {
        ...
        for (let key in defaultTeamsMapping) {
          if ((student.name && student.name.includes(key))) {
      ...
      window.renderTeamsView = function(containerId, isStudentView = false) {
        ...
        for (let t in teamsData) {
          ...
          if (isStudentView && currentUser) {
            ...
            if (myTeam === t) {
              ctaHtml = `<a href="${whatsappLinks[t]}" target="_blank" class="team-cta" onclick="if(this.getAttribute('href')==='#') { alert('عذراً، الرابط قيد التجهيز من الإدارة!'); return false; }">💬 انضم لتيمك على الواتساب</a>`;
            }
          }
...
if "defaultTeamsMapping" not in content:
    content = content.replace('      async function renderTeacher() {', js_logic + '\n      async function renderTeacher() {')

# Render teams in dashes
if "renderTeamsView('t-teams-container');" not in content:
    content = content.replace("g('t-analytics-container').innerHTML = analyticsHtml ||", "renderTeamsView('t-teams-container');\n        g('t-analytics-container').innerHTML = analyticsHtml ||")
if "renderTeamsView('s-teams-container', true);" not in content:
    content = content.replace('g("s-tasks-badge").style.display = stc > 0 ? "inline" : "none";', 'g("s-tasks-badge").style.display = stc > 0 ? "inline" : "none";\n        renderTeamsView("s-teams-container", true);\n')
